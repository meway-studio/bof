...
class DefaultController extends Controller
{
	...
	public function actionSpam(){
		...
		if(isset($_POST['spamId'])){
			...
			SWITCH($_POST['spamId']){
				CASE 'signup':  $this->sendmailSignup($model);  break;
				CASE 'confirm': $this->sendmailConfirm($model); break;
				CASE 'forgot':  $this->sendmailForgot($model, new UserForgot('create'));  break;
				CASE 'restore': $this->sendmailRestore($model); break;
				CASE 'widget' : $this->sendmailWidget($model); break;
			}
		}
	...
	public function actionSignup(){
		...
		if(isset($_POST['User'])){
			...
			if(!$model->hasErrors() AND $model->save()){
				...
				Yii::app()->user->setFlash('signupSuccess',Yii::t('user','Поздравляю, Регистрация прошла успешно! Проверьте вашу почту.'));
	...
	public function actionConfirm($hash){
		...
		if($model!==null AND $model->confirm==0){
			...
			if(!$model->hasErrors() AND $model->save()){
				...
				Yii::app()->user->setFlash('confirmSuccess',Yii::t('user','Ваша учетная запись была активирована. Проверьте вашу почту. Добро пожаловать в команду BetOnFootball!'));
			}else{
	...
	*/
		...
		if(isset($_POST['User'])){
			...
			if(!$model->hasErrors() AND $model->save()){
				...
				if($model->confirm==0){
					...
					$this->sendmailSignup($model);
					Yii::app()->user->setFlash('updateSuccess',Yii::t('user','Данные были изменены. Вам отправлено письмо на указанный E-mail. Для активации E-mail пройдите по ссылке в письме.'));
	...
	protected function sendmailSignup($model){
		...
		$message       = new YiiMailMessage(Yii::t('user','Регистрация на сайте BoF').' '.Yii::app()->name);
		$message->view = Yii::app()->language.'/signup';
	...
	protected function sendmailConfirm($model){
		...
		$message       = new YiiMailMessage(Yii::t('user','Ваша учетная запись активирована').' '.Yii::app()->name);
	...
	protected function sendmailForgot($user, $forgot){
		...
		$message       = new YiiMailMessage(Yii::t('user','Восстановление пароля для входа на BoF').' '.Yii::app()->name);
		$message->view = 'forgot';
		$message->setBody(array('model'=>$user, 'forgot'=>$forgot), 'text/html');
	...
	protected function sendmailRestore($model){
		...
		$message       = new YiiMailMessage(Yii::t('user','Ваш новый пароль для входа на BoF').' '.Yii::app()->name);
		$message->view = 'restore';
		$message->setBody(array('model'=>$model), 'text/html');
		$message->addTo( $model->email );
		$message->from = array(Yii::app()->config->get('EMAIL_NOREPLY') => Yii::app()->name);
		return Yii::app()->mail->send($message);
	...
	protected function sendmailWidget($model){
		...
		$message       = new YiiMailMessage(Yii::t('user','Новый ').' '.Yii::app()->name);
		$message->view = 'widget';
		$message->setBody(array('model'=>$model), 'text/html');
		$message->addTo( $model->email );
		$message->from = array(Yii::app()->config->get('EMAIL_NOREPLY') => Yii::app()->name);
