...
class Transliteration
{
            ...
            for ($i = -1; --$len;) {
                ...
                if ($remaining = $tail_bytes[ $c ]) {
                    ...
                    if ($n <= 0xdf) {
                        ...
                        $ord = ($n - 240) * 262144 + (ord( $sequence[ 1 ] ) - 128) * 4096 + (ord( $sequence[ 2 ] ) - 128) * 64 + (ord(
                                    $sequence[ 3 ]
                                ) - 128);
                    } elseif ($n <= 0xfb) {
                        $ord =
                            ($n - 248) * 16777216 + (ord( $sequence[ 1 ] ) - 128) * 262144 + (ord( $sequence[ 2 ] ) - 128) * 4096
                            + (ord( $sequence[ 3 ] ) - 128) * 64
                            + (ord( $sequence[ 4 ] ) - 128);
                    } elseif ($n <= 0xfd) {
                        $ord =
                            ($n - 252) * 1073741824 + (ord( $sequence[ 1 ] ) - 128) * 16777216 + (ord( $sequence[ 2 ] ) - 128) * 262144
                            + (ord( $sequence[ 3 ] ) - 128) * 4096 +
                            (ord( $sequence[ 4 ] ) - 128)
                            * 64 + (ord( $sequence[ 5 ] ) - 128);
                    }
    ...
    public static function url( $url, $unknown = '?', $langcode = null )
    {
        $url = self::text($url, $unknown, $langcode);
        $url = preg_replace(array('/\W/', '/^(\s+|\-+)/', '/(\-+|\s+)$/', '/\-+/'), array('-', '', '', '-'), $url);
        return $url;
    }
}
