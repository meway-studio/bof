...
class Tipsters extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, stats, tips, articles, count_active, count_won, count_void, count_lost, editor', 'numerical', 'integerOnly'=>true),
			array('profit, yield, winrate, odds, bank', 'length', 'max'=>10),
			array('meta_k, meta_d', 'length', 'max'=>250),

			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, rank, comment, stats, tips, articles, profit, yield, winrate, odds', 'safe', 'on'=>'search'),
		);
	}
	...
	public function getActiveCount(){
		$count = Yii::app()->db->createCommand()
			->select('COUNT(*) AS `c`')
			->from('{{tips}}')
			->where('tipster_id=:ID AND calculated=0 AND status=:S', array(':ID'=>$this->user_id, ':S'=>Tips::STATUS_ACTIVE))
			->queryRow();
