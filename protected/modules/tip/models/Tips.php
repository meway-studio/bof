...
class Tips extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			*/
			...
			// create
			array('status, type, club_1, flag_1, club_2, flag_2, description, content, selection, odds, stake,tipster_id', 'required', 'on'=>'create'),
			array('status, type, tip_result, bet_on, calculated, event_h, event_m, bod, bookmaker', 'numerical', 'integerOnly'=>true, 'on'=>'create'),
			...
			array('status, type, tip_result, bet_on, calculated, tipster_id, event_date, create_date', 'numerical', 'integerOnly'=>true, 'on'=>'import'),
			...
			array('id, status, type, price, create_date, update_date, event_date, tipster_id, club_1, flag_1, club_2, flag_2, description, content, cover, selection, odds, stake, profit, tip_result, match_result', 'safe', 'on'=>'search'),
		);
	}
	...
	protected function afterSave(){
		...
		if($this->calculated==1){
			...
			$wonCount = Tips::model()->byTipster($this->tipster_id)->count('tip_result=:R1 OR tip_result=:R2', array(':R1'=>Tips::TIP_RESULT_WON, ':R2'=>Tips::TIP_RESULT_HALF));
			...
			$odds = Yii::app()->db->createCommand()->select('SUM(`odds`) AS `sum`')->from('{{tips}}')->where('tipster_id=:id', array(':id'=>$this->tipster_id))->queryRow();
	...
	protected function calcStatistic(){
		...
		$stakeSum = Yii::app()->db->createCommand()->select('SUM(`stake`) AS `sum`')->from('{{tips}}')->where('tipster_id=:ID', array(':ID'=>$this->tipster_id))->queryRow();
	...
	public function getFormatPrice(){
		return $this->isFree ? Yii::t('tips', 'Бесплатно') : Yii::t('tips', '&euro;&nbsp;{price}', array('{price}'=>$this->getViewPrice()));
	}
	...
	public function getAccessManage(){
		...
		}else if($this->tipster_id == Yii::app()->user->id){
			return true;
		
		}else if(Yii::app()->user->isEditor){
			return true;
			
		}else if(Yii::app()->user->RoleId == User::ROLE_MODERATOR){
			return true;
			
		}else if(Yii::app()->user->RoleId == User::ROLE_ADMIN){
			return true;
	...
	public function getBestOdds(){
		...
		$model = Yii::app()->db->createCommand()
			->select('MAX(`odds`) AS `best`, `firstname`')
			->from( $this->tableName().' tp')
			->join('me_users u', 'u.id=tp.tipster_id')
			->where('event_date=:E AND club_1=:C1 AND club_2=:C2', array(':E'=>$this->event_date, ':C1'=>$this->club_1, ':C2'=>$this->club_2))
			->queryRow();
		
		if(!isset($model['best']))
			return CHtml::tag('span', array('class'=>'not-found'), Yii::t('tips', 'Нет данных'));
			
		return Yii::t('tips', '{odds} {at} {betfair}', array(
	...
	public function getBetOnClub(){
		...
		switch($this->bet_on)
		{
			CASE 0: return $this->selection; break;
			CASE 1: return $this->club_1; break;
			CASE 2: return $this->club_2; break;
		}
	...
	public function getTempProfit(){
		SWITCH($this->tip_result){
			CASE self::TIP_RESULT_WON:  return $this->odds*$this->stake-$this->stake;
			CASE self::TIP_RESULT_LOST: return $this->stake*-1;
			CASE self::TIP_RESULT_HALF_LOST: return (($this->stake*-1) / 2);
			CASE self::TIP_RESULT_DRAW: return 0;
			CASE self::TIP_RESULT_HALF: return ($this->odds*$this->stake-$this->stake)/2;
			default: return 0;
		}
	}
	...
	public function dateWithTime(){
		...
		$this->event_date = strtotime($tmp[2]."-".$tmp[1]."-".$tmp[0].' '.$this->event_h.':'.$this->event_m.':00');
	}
	...
	// title= Club vs Club Tip and Preview Date (day month year) from Tipster Country League Football betting tips
		...
		return Yii::t('Tips', '{c1} & {c2} Совет и Предварительный просмотр {ed} от {fn} - {leag} Футблольных ставок', array('{c1}'=>$this->club_1,'{c2}'=>$this->club_2,'{ed}'=>Yii::app()->dateFormatter->format('dd MMMM yyyy', $this->event_date),'{fn}'=>$this->tipster->FullName,'{leag}'=>$this->league));
	}
	...
	public function getBookmakerImage(){
		...
		{
			case self::BOOKMAKER_BETFAIR:  return '/css/images/betfair_logo.png';
			case self::BOOKMAKER_SBOBET:   return '/css/images/sb.png';
			case self::BOOKMAKER_BET365:   return '/css/images/365.png';
			case self::BOOKMAKER_PINNACLE: return '/css/images/pin.png';
			default: return '/css/images/logo.png';
		}
	}
}