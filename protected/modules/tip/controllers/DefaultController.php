...
class DefaultController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions' => array('index', 'list', 'view', 'tipsters', 'subscription', 'stat', 'allStat', 'ajaxmore', 'NoBetTips', 'NbView', 'Robokassa'),
				'users'   => array('*'),
			),
	...
	public function actionRobokassa(){
		...
		Yii::app()->robokassa->onSuccess = function($event){
			...
			if($model->status == Purchase::STATUS_NEW){
				...
				// или увеличить время
				}else if($model->type == Purchase::TYPE_DATE){
					...
					$user->sub->expiration_date = (($user->sub->expiration_date>date('U')) ? ($user->sub->expiration_date+$model->days) : (date('U')+$model->days));
					$user->sub->save();
				}
			}
	...
	public function actionNbView($id){
		...
		$tips = Tips::model()->published()->closed()->byTipster($model->tipster_id)->findAll(array('order'=>'create_date DESC', 'limit'=>7));
		$last = array('won'=>0, 'lost'=>0, 'void'=>0);

		foreach ($tips as $item) {
			SWITCH($item->tip_result){
				CASE Tips::TIP_RESULT_WON:  $last['won']++;  break;
				CASE Tips::TIP_RESULT_HALF: $last['won']++;  break;
				CASE Tips::TIP_RESULT_LOST: $last['lost']++; break;
				CASE Tips::TIP_RESULT_VOID: $last['void']++; break;
			}
		}
	...
	public function actionCart($confirm=null){
		...
		if($confirm==1){
				...
				if(!$model_p->hasErrors()){
					...
					if($status){
						...
						if($model_p->IsRobokassa){
							Yii::app()->robokassa->pay($model_p->PriceRUR, $model_p->id, CHtml::encode($model_p->description), $model_p->user->email, $model_p->IncCurrLabel);
						}
			...
			$result['totalPrice'] = Yii::app()->language=='ru' ? $result['totalPrice']*Yii::app()->params->rur_eur : $result['totalPrice'];
			
			$this->render('buy_subscription', array(
				'model'  => $model_p,
				'price'  => $result['totalPrice'],
				'status' => $status,
				'termDesc' => Yii::t('tips', Yii::t('tips', '{cm} Совет|{cm} Советa|{cm} Советов|{cm} Совета', count($model)), array('{cm}'=>count($model))),
			));
	...
	public function actionBuySubscription($term, $success=false){
		...
		{
			CASE 'weekend': $date = 7*24*60*60;    $termDesc = Yii::t('themes', 'Неделя подписки');     $price = Yii::app()->config->get('SUBSCRIPTION_WEEKEND_PRICE'); break;
			CASE 'month':   $date = 30*24*60*60;   $termDesc = Yii::t('themes', '1 месяц подписки');    $price = Yii::app()->config->get('SUBSCRIPTION_MONTH_PRICE');   break;
			CASE '3month':  $date = 90*24*60*60;   $termDesc = Yii::t('themes', '3 месяца подписки');   $price = Yii::app()->config->get('SUBSCRIPTION_3MONTH_PRICE');  break;
			CASE 'season':  $date = 240*24*60*60;  $termDesc = Yii::t('themes', '9 месяцев подписки');  $price = Yii::app()->config->get('SUBSCRIPTION_SEASON_PRICE');  break;
		}
		...
		if(isset($_POST['Purchase'])){
			...
			if(!$model->hasErrors()){
				...
				if($model->IsRobokassa){
					Yii::app()->robokassa->pay($model->PriceRUR, $model->id, CHtml::encode($termDesc), $model->user->email, $model->IncCurrLabel);
	...
	public function actionStat($id){
		...
		$dataProvider = new CActiveDataProvider(Tipstats::model()->byTipster($model->id)->toGrid(), array('pagination'=>array('pageSize'=>Tips::TIP_LAST_COUNT)));
		
		$this->render('tipster_stat', array(
			'model' => $model,
			'chart' => $chart,
			'dataProvider' => $dataProvider,
		));
	...
	public function actionAllStat(){
		...
		foreach ($model as $item) {
			...
			$allStake = Yii::app()->db->createCommand()->select('SUM(`stake`) AS `sum`')->from('{{tips}}')->where('tipster_id=:ID', array(':ID'=>$item->id))->queryRow();
		...
		$dataProvider = new CActiveDataProvider(Tipstats::model()->byTipster(0)->toGrid(), array('pagination'=>array('pageSize'=>Tips::TIP_LAST_COUNT)));
		
		$this->render('all_stat', array(
			'chart'        => $chart,
			'dataProvider' => $dataProvider,
			'bof'          => $bof,
		));
