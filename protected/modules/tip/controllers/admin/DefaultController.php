...
class DefaultController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions'=>array('admin','delete', 'create','update', 'index','view', 'settings', 'purchase', 'PurchaseDetail', 'purchaseDelete', 'adminInput', 'meta', 'calc', 'statistics'),
                'roles'=>array('admin'),
            ),
    ...
    public function actionCalc(){
    	...
    	// Получить типсы за период
    	$model = Tips::model()->published()->closed()->findAll($c);

    	$result['count_all'] = count($model);
    	...
    	foreach($model AS $item){
			...
			SWITCH($item->tip_result){
				CASE Tips::TIP_RESULT_WON:  $result['count_won']++;  break;
				CASE Tips::TIP_RESULT_HALF: $result['count_won']++;  break;
				CASE Tips::TIP_RESULT_LOST: $result['count_lost']++; break;
				CASE Tips::TIP_RESULT_VOID: $result['count_void']++; break;
				CASE Tips::TIP_RESULT_HALF_LOST: $result['count_lost']++; break;
			}
    	}
	...
	public function actionPurchaseDetail($id, $action=null){
		...
		$this->render('purchase_detail', array(
			'model' => $model,
		));
	}
