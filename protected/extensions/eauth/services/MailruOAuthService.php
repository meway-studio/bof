...
class MailruOAuthService extends EOAuth2Service {
	...
	public function makeSignedRequest($url, $options = array(), $parseJson = true) {
		if (!$this->getIsAuthenticated()) {
			throw new CHttpException(401, Yii::t('eauth', 'Не удается завершить проверку подлинности, так как необходимые данные не были получены.', array('{provider}' => $this->getServiceTitle())));
		}
