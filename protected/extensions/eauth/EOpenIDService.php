	...
	public function authenticate() {
		if (!empty($_REQUEST['openid_mode'])) {
			switch ($_REQUEST['openid_mode']) {
						...
						if ($this->auth->validate()) {
							...
							foreach ($this->requiredAttributes as $key => $attr) {
								if (isset($attributes[$attr[1]])) {
									...
									throw new EAuthException(Yii::t('eopen', 'Не удается завершить проверку подлинности, так как не были получены необходимые данные.', array('{provider}' => $this->getServiceTitle())));
									return false;
								}
							}
				...
				default:
					throw new CHttpException(400, Yii::t('eopen', 'Ваш запрос является недействительным.'));
					break;
			}
		}
		else {
