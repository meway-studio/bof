...
abstract class EAuthServiceBase extends CComponent implements IAuthService {
	...
	protected function makeRequest($url, $options = array(), $parseJson = true) {
		...
		if ($headers['http_code'] != 200) {
			...
			throw new EAuthException(Yii::t('eauth', 'Неверный ответ кода http: {code}.', array('{code}' => $headers['http_code'])), $headers['http_code']);
		}
	...
	protected function parseJson($response) {
		try {
			...
			if (!isset($result)) {
				throw new EAuthException(Yii::t('eauth', 'Неверный формат ответа.', array()), 500);
			}
			else {
	...
	protected function fetchJsonError($json) {
		if (isset($json->error)) {
			return array(
				'code' => 500,
				'message' => Yii::t('eauth', 'Неизвестная ошибка'),
			);
		}
		else {
			return null;
