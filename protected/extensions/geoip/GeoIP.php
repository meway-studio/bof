...
// Copyright 2009 Maxmind LLC All Rights Reserved
  ...
  public function lookupRegion($addr) {
    $ipnum = ip2long($addr);
    if ($ipnum === false) {
      throw new CException(Yii::t('geoip', 'Неверный IP адрес: {addr}', array('{addr}' => var_export($addr, true))), self::ERR_INVALID_IP);
    }
    if ($this->databaseType !== self::REGION_EDITION_REV0 && $this->databaseType !== self::REGION_EDITION_REV1) {
      throw new CException(Yii::t('geoip', 'Неверный тип базы данных; Метод lookupLocation() не может найти таблицу Region в базе данных'), self::ERR_DB_FORMAT);
    }
  ...
  public function lookupLocation($addr) {
    ...
    if ($this->databaseType !== self::CITY_EDITION_REV0 && $this->databaseType !== self::CITY_EDITION_REV1) {
      throw new CException(Yii::t('geoip', 'Неверный тип базы данных; Метод lookupLocation() не может найти таблицу City в базе данных'));
    }
