...
$this->menu = array();
$this->menu[ ] = array(
    'label' => Yii::t( 'CatalogModule.admin.element.main', 'Просмотр' ),
    'url'   => Yii::app()->createUrl( 'catalog/admin/category/index', array( 'id' => $model->category_id ) )
);
if (!$model->isNewRecord) {
    $this->menu[ ] = array(
        'label' => Yii::t( 'CatalogModule.admin.element.main', 'Создать элемент' ),
...
$this->breadcrumbs[ ] = Yii::t(
    ...
    ($model->isNewRecord ? 'Добавление' : 'Редактирование') . ' элемента'
);

?>

    <h3><?= Yii::t( 'CatalogModule.admin.element.main', ($model->isNewRecord ? 'Добавление' : 'Редактирование') . ' элемента' ) ?></h3>
<?php echo $this->renderPartial( 'articles/form', array( 'model' => $model ) ); ?>